...
function PlatformSelect({ onChange, selectedPlatform }) {
  ...
  return (
    <PlatformSelectContainer ref={wrapperRef}>
      <button
        className={`round-borders wrapper${
          isDropdownVisible ? " hovered-wrapper" : ""
        }`}
      ...
      {isDropdownVisible && (
        <ul className="options round-borders with-shadow">
          {Object.values(settings.platforms).map(({ title, name }) => (
            <li
              key={name}
              className={`option-item round-borders${
                name === selectedPlatform.name ? " selected" : ""
              }`}
            >
              <button type="button" onClick={() => handleChange(name)}>
                <div
                  ...
                  style={{
                    backgroundColor: themeContext[name].titleBackgroundColor
                  }}
                />
